...
def make_adapter() -> tuple[ClaudeSessionAdapter, FakeClient]:
    ...
    context = ClaudeSessionContext(
        ...
        scope=ProviderScope(user_identity_id="user-1", project_id="project-1"),
    )
...
def test_adapter_enforces_context_scope() -> None:
    ...
    adapter.recall(RecallRequest(scope=ProviderScope(project_id="other")))
    assert client.recalls[-1].scope.project_id == "project-1"
