...
def get_scope_revisions(user_identity_id: UUID | None, project_id: UUID | None, task_id: UUID | None) -> dict[str, int]:
    ...
    scope_types = [scope_type for scope_type, _ in scopes]
    scope_ids = [scope_id for _, scope_id in scopes]
    with get_db_cursor() as cursor:
        cursor.execute(
            """
            INSERT INTO context_revision (scope_type, scope_id, revision)
            SELECT scope_type, scope_id, 1
            FROM UNNEST(%s::text[], %s::uuid[]) AS requested(scope_type, scope_id)
        ...
        cursor.execute(
            ...
            FROM context_revision revision
            JOIN UNNEST(%s::text[], %s::uuid[]) AS requested(scope_type, scope_id)
              ON requested.scope_type = revision.scope_type
             AND requested.scope_id = revision.scope_id
            """,
            (scope_types, scope_ids),
        )
        return {
            f"{row['scope_type']}:{row['scope_id']}": int(row["revision"])
...
def _fetch_project_context(cursor, project_id: UUID, include_history: bool, limit: int, data: dict) -> None:
...
def _fetch_assertions(cursor, subject_ids: list[UUID], include_history: bool, limit: int) -> list[dict]:
    if include_history:
        cursor.execute(
            """
...
def fetch_structured_context(project_id: UUID | None, task_id: UUID | None, include_history: bool, limit: int) -> dict:
    ...
    with get_db_cursor() as cursor:
        if project_id:
            _fetch_project_context(cursor, project_id, include_history, limit, data)

        subject_ids = [value for value in (project_id, task_id) if value]
        if subject_ids:
            data["assertions"] = _fetch_assertions(cursor, subject_ids, include_history, limit)
